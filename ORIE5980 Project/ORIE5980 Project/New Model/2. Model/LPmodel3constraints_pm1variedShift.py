...
def pm(pattern,window):
    ...
    if len(pattern)==1:
    	...
    	spread = pattern[1] - pattern[0]
        pool = [range(pattern[i]-window, pattern[i]+window+1) for i in xrange(len(pattern))]
        patternOp = [(a,b) for a in pool[0] for b in pool[1] if abs((b-a) - spread) <= 1]
        patternOp = [(a,b) for (a,b) in patternOp if a and b in range(1,nEvenings+1)]
    else:
    	spread1 = pattern[1] - pattern[0]
    	spread2 = pattern[2] - pattern[1]
        pool = [range(pattern[i]-window,pattern[i]+window+1) for i in xrange(len(pattern))]
        patternOp = [(a,b,c) for a in pool[0] for b in pool[1] for c in pool[2] if abs((b-a) - spread1) <= 1 and abs((c-b) - spread2) <= 1]
