		...
		for i in xrange(n-1):
			...
			for j in xrange(i+1,n):
				...
				if conflictsBetweenTheseTwo > 0:
					print evening, "courses:", c1, c2, "conflicts:", conflictsBetweenTheseTwo, y[c1-1][c2-1][evening-1]


for key in conflictsPerCourse.keys():
	print key, conflictsPerCourse[key]
	
sameEveningCourses = [(2*i-1,2*i) for i in xrange(1,nEvenings+1)]
nBackToBacks = 0
for e1,e2 in sameEveningCourses:
	for c1 in schedule[e1]:
		for c2 in schedule[e2]:
			nBackToBacks +=  O[c1-1,c2-1]